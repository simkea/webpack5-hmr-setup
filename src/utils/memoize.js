...
const defaultMemoize = (MapCache, resolver, func) => {
    if (typeof resolver !== 'function' || typeof func !== 'function') {
        throw new TypeError('Memoization expects parameters to be functions.');
    }
 ...
 */
export const memoize = memoizeWith(WeakMap);
 ...
 * @returns {string}
 */
memoizeHash.stringify = (...args) => args.reduce((str, val) => `${str}_${JSON.stringify(val)}`, '');
